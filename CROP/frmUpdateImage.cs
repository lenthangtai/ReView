...
namespace VCB_TEGAKI
{
    public partial class frmUpdateImage : Form
    {
        ...
        int getid;
        int iturn = 0;
        string path = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\path_plusNCC.txt";
        ...
        private void btnSourceDirectoryBrow_Click(object sender, EventArgs e)
        {
            ...
            if (rdb_pdf.Checked == true)
            {
                ...
                using (var fbd = new FolderBrowserDialog())
                {
                    ...
                    {
                        //folder_extra = Path.GetDirectoryName(fbd.SelectedPath) + "\\" + DateTime.Now.ToString("yyyyMMdd_hhmmss") + "_" + Path.GetFileName(fbd.SelectedPath).Replace(" ","");
                        folder_extra = fbd.SelectedPath + "\\" + Path.GetFileName(fbd.SelectedPath).Replace(" ", "");
                        str8 = Path.GetFileName(fbd.SelectedPath).Split('-')[0];
                        if (str8.Length != 8)
                        { MessageBox.Show("folder sai quy tắc, yyyymmdd-xxx, trước dấu - không đủ 8 ký tự"); return; }
                        DateTime dtresult;
                        if (!DateTime.TryParseExact(str8, "yyyyMMdd", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out dtresult))
                        { MessageBox.Show("8 ký tự đầu không dúng định dạng không đúng yyyymmdd"); return; }
                        arrpdf = Directory.GetFiles(fbd.SelectedPath, "*.pdf", SearchOption.AllDirectories);
                        txtSourceDirectory.Text = fbd.SelectedPath;
                    }
                    else
                    { return; }
                }
                int lng = arrpdf.Length;
                if (lng > 0)
                {
                    Directory.CreateDirectory(folder_extra);
                    for (int i = 0; i < lng; i++)
                    {
                        int stt = 1;
                        PdfiumViewer.PdfDocument document = PdfiumViewer.PdfDocument.Load(arrpdf[i].ToString());
                        string filename = Path.GetFileName(arrpdf[i].ToString());
                        for (int a = 0; a < document.PageCount; a++)
                        {
                            Image image;
                            //string imagename = filename.Split('.')[0].ToString() + " VBPO-" + stt_img.ToString().PadLeft(4,'0');
                            string imagename = filename.Replace(".pdf","†").Split('†')[0] + "_ket" + str8 + "kem" + stt.ToString().PadLeft(4, '0');
                            if (document.PageSizes[a].Width < 800 && document.PageSizes[a].Height < 1200)
                            {
                                try
                                {
                                    ...
                                    //image.Save(folder_extra + "\\" + filename.Split('.')[0].ToString().Trim() + @"\" + imagename + ".jpeg");
                                    image.Save(folder_extra + "\\" + imagename + ".jpeg");
                                    image.Dispose();
                                }
                ...
                { MessageBox.Show("Khôn chộ file PDF mô bạn ey!"); return; }
            }
            else
            {
            ...
            pagesFilePath = Directory.GetFiles(folder_extra, "*.jpeg", SearchOption.AllDirectories);
            int sl = pagesFilePath.Length;
            List<string> lstfilename = new List<string>();
            for (int i = 0; i < sl; i++)
            {
                //check trung
                string filename = ToFullWidth(Path.GetFileName(pagesFilePath[i]));
                if (lstfilename.IndexOf(filename) < 0)
        ...
        static string ToFullWidth(string halfWidth)
        {
        ...
        private void frmUpdateImage_Load(object sender, EventArgs e)
        {
            ...
            //string str_linkUp = @"D:\C#\Phung\All_App\NNC\20221004-ca3-1";
            //pagesFilePath = Directory.GetFiles(str_linkUp, "*.jpeg", SearchOption.AllDirectories);

            //for (int i = 0; i < pagesFilePath.Length; i++)
            //{
            //    int idimg = dAEntry.GetIntSQL("select Id from dbo.ServerImage where NameImage = N'" + Path.GetFileName(pagesFilePath[i]) + "'");
            //    if (idimg <= 0)
            //    {
            //        Bitmap imagepage = new Bitmap(pagesFilePath[i]);
            //        dbsql.InsertImageToServer(Path.GetFileName(pagesFilePath[i]), imageToByteArray(imagepage));
